...
Como podemos en el gráfico hay datos que se desvían demasiado de los
cuantiles teóricos de la distribución normal, lo cual es una señal
grande para dudar de la normalidad de los residuales.

Además, realizamos el test de Shapiro-Wilk donde la hipótesis nula es
...
influencial si $|DFBETAS_{j(i)}| > 2/\sqrt{n}$, en este caso nuestro
...
Como podemos ver en la grifica datos hay varios datos que superan el
...
esto en mente, en orden, los datos mas potencialmente influencíales de
acuerdo a esta medida son: 34, 47, 17, 46
...
En esta gráfica se encuentran los hat-values vs. residuales
estudentizados, como podemos observar la gráfica usa un limite
diferente, en esta el limite es 0.12 y hemos visto en clase el limite
que fijamos es de $\pm3$, pero este otro calculado de una forma
diferente nos puede dar un indicio a cuales podrían ser atípicas.

Igualmente podemos observar que los puntos de balance pueden ser 34 y
82, basado en el valor $h_{ii}$
